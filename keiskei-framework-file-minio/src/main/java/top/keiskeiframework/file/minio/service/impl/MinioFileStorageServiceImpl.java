...
@Component
@Slf4j
public class MinioFileStorageServiceImpl implements FileStorageService, ProgressListener {
    ...
    private String getUploadId(String fileName) {
        ...
        if (cacheStorageService.exist(key)) {
            ...
            InitiateMultipartUploadRequest request = new InitiateMultipartUploadRequest(fileMinioProperties.getBucket(), fileName);
            InitiateMultipartUploadResult result = minioClient.get(request);
    ...
    public void uploadPart(MultiFileInfo fileInfo) {
        ...
        try {
            ...
            uploadPartRequest.setBucketName(fileMinioProperties.getBucket());
            ...
            UploadPartResult uploadPartResult = minioClient.uploadPart(uploadPartRequest);
            ...
            if (!StringUtils.isEmpty(fileName)) {
                ...
                if (StringUtils.isEmpty(uploadId)) {
                    ...
                    AbortMultipartUploadRequest abortMultipartUploadRequest =
                            new AbortMultipartUploadRequest(fileMinioProperties.getBucket(), fileName, uploadId);

                    minioClient.abortMultipartUpload(abortMultipartUploadRequest);
                }
            }
    ...
    public FileInfo mergingChunks(MultiFileInfo fileInfo) {
        ...
        try {
            ...
            CompleteMultipartUploadRequest completeMultipartUploadRequest =
                    new CompleteMultipartUploadRequest(fileMinioProperties.getBucket(), fileName, uploadId, partEtags);
    ...
    public FileInfo exist(String fileName) {
        if (minioClient.doesObjectExist(fileMinioProperties.getBucket(), fileName)) {
    ...
    public void delete(String fileName) {
        minioClient.deleteObject(fileMinioProperties.getBucket(), fileName);
    }

    @Override
    public void deleteBatch(List<String> fileNames) {
        minioClient.deleteObjects(new DeleteObjectsRequest(fileMinioProperties.getBucket()).withKeys(fileNames));
    }

    @Override
    public InputStream download(String fileName) {
        return minioClient.getObject(fileMinioProperties.getBucket(), fileName).getObjectContent();
    }

    @Override
    public void show(String fileName, String process, HttpServletResponse response) {
        GetObjectRequest getObjectRequest = new GetObjectRequest(fileMinioProperties.getBucket(), fileName);
        if (!StringUtils.isEmpty(process)) {
            getObjectRequest.setProcess(process);
        }
        OSSObject ossObject = minioClient.getObject(getObjectRequest);
