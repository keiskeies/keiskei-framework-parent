...
public class ProjectController extends ListController<ProjectInfo> {
    ...
    @ApiOperation("创建代码")
    public R<Boolean> build(@PathVariable("id") String id) {
    ...
    @ApiOperation("代码创建状态")
    public R<BuildStatusEnum> status(@PathVariable("id") String id) {
        return R.ok(generateService.refreshStatus(id));
    }

    @GetMapping("/{id}/download")
    @ApiOperation("代码下载地址")
    public R<String> download(@PathVariable("id") String id) {
