...
<#list table.fields as field>
  <#if !field.jsonIgnore>
          {key: '${field.name}', label: '${field.comment}', show: ${field.directShow?string}, edit: ${field.editAble?string}, queryable: ${field.queryAble?string}, sortable: ${field.sortAble?string}, minWidth: ${field.tableWidth},type:<#if field.type == "MIDDLE_ID"><#if field.relation == "ONE_TO_ONE" || field.relation == "MANY_TO_ONE"><#if treeTables?seq_contains(field.relationEntity)>"TREE_SELECT",<#else>"SELECT",</#if><#elseif field.relation == "ONE_TO_MANY" || field.relation == "MANY_TO_MANY" ><#if treeTables?seq_contains(field.relationEntity)>"TREE_MULTI_SELECT",<#else>"MULTI_SELECT",</#if></#if><#else>'${field.type}',</#if><#if field.type == "MIDDLE_ID">optionKey: '${field.relationEntity?uncap_first}Options', <#elseif field.type == "DICTIONARY">optionKey: '${table.name?uncap_first}${field.name?cap_first}Options', </#if>tooltip: '${field.tooltip!}'},
  </#if>
</#list>
        ],
        format: {
